...
public class GreetClient {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Please enter command parameters (format: <PROTOCOL PHASE> <MEASUREMENT TYPE> <NUMBER OF PROBES> <MESSAGE SIZE> <SERVER DELAY>)");
    ...
    private static String[] checkMPCommond(GreetClient client, Scanner scanner) throws IOException{
        System.out.println("Please enter input in the form <PROTOCOL PHASE><WS><PROBE SEQUENCE NUMBER><WS><PAYLOAD> for MP testing");
        String userInput = scanner.nextLine();
        ...
        while (true){
            if (tokens.length == 3 && "m".equalsIgnoreCase(tokens[0])) {
                ...
                System.err.println("Numeric parameter format error. Please enter input in the form <PROTOCOL PHASE><WS><PROBE SEQUENCE NUMBER><WS><PAYLOAD>");
    ...
    // Perform RTT measurement
    private static Boolean performRTT(GreetClient client, int probes, int size, int delay, Scanner scanner) throws IOException {
        ...
        for (int i = 0; i < probes; i++) {
            ...
            rttTimes[i] = (endTime - startTime) / 1000000.0; // Convert to milliseconds
            ...
            if (response != null && !response.isEmpty()) {
                System.out.println("Received reply: " + response + " | RTT: " + rttTimes[i] + " ms");
            } else {
                System.out.println("No reply received | RTT: N/A");
            }
        ...
        client.out.println("RTT test ended"); // This should probably be in English
        ...
        System.out.println("RTT measurement completed. Average RTT: " + averageRTT + " ms");
        return true;
    }

    // Perform Throughput measurement
    private static boolean performTput(GreetClient client, int probes, int size, int delay, Scanner scanner) throws IOException {
        ...
        double tput = (totalBytes / 1024.0) / (duration / 1000.0); // KB/s
        client.out.println("throughput measurement completed");
        String response = client.in.readLine();
        System.out.println("Received reply: " + response);
        System.out.println("Throughput measurement completed. Total data: " + totalBytes + " bytes, Time: " + duration + " ms, Throughput: " + tput + " KB/s");
        return true;
    }
